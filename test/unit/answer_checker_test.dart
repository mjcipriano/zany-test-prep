...
Question _spr({num value = 12, List<String>? accepted, double tol = 0}) => Question(
      id: 'q2',
      examId: 'sat',
      domain: 'math',
      section: 'algebra',
...
void main() {
  ...
  group('student produced numeric', () {
    ...
    test('within tolerance', () {
      expect(checker.checkProduced(_spr(value: 3.14, tol: 0.01), '3.15'), isTrue);
      expect(checker.checkProduced(_spr(value: 3.14, tol: 0.01), '3.2'), isFalse);
    });
    test('fraction equivalent to decimal', () {
      expect(checker.checkProduced(_spr(value: 0.5, accepted: const ['1/2']), '1/2'),
          isTrue);
      expect(checker.checkProduced(_spr(value: 0.5, accepted: const ['0.5']), '1/2'),
          isTrue);
    });
